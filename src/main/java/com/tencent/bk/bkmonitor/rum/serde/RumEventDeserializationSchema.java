...
import com.tencent.bk.bkmonitor.rum.utils.QpsMetrics;
...
public class RumEventDeserializationSchema
  ...
  /** 输入 QPS 计数(rum-events-source.qps.input);未注册时 mark 为空操作,测试可安全 new 本类。 */
  private transient QpsMetrics inputQps;
  ...
  /** 成功走 flat event 兼容路径的消息数。 */
  private transient Counter flatEventCounter;
  ...
  public void open(InitializationContext context) {
    inputQps = QpsMetrics.register(context.getMetricGroup(), "input");
    ...
    flatEventCounter = serdeMetrics.counter("flat_events");
  }
  ...
  private void deserializeValidJson(byte[] message, List<RumEvent> events) throws IOException {
    try (JsonParser p = MAPPER.createParser(message)) {
      ...
      if (flatEventCounter != null) {
        flatEventCounter.inc();
      }
