...
public final class RumJobTopology {
  ...
  public static void build(StreamExecutionEnvironment env, RumJobConfig config) {
    KafkaSource<RumEvent> source = createSource(config.kafka());
  ...
  private static KafkaSource<RumEvent> createSource(RumJobConfig.KafkaConfig config) {
    ...
    return KafkaSource.<RumEvent>builder()
        ...
        .setProperty("client.id.prefix", "rum-session-precompute-")
  ...
  private static void writeLateEventSink(
    ...
    // NONE 模式故意不注册 sink，用于当前处理时间窗口不产生迟到数据的部署。
  }
